...
// My code
	...
	function renderNewMember(req,res){
	...
	function addNewMember(req, res,next) { 
		...
		//validate req
		var userData = JSON.parse(JSON.stringify(req.body)); //deep copy
		var uid = userData.name + '.' + userData.sn;
		userData.cn = userData.name + ' ' + userData.sn;
		userData.bodyCategory = 'Local';
		userData.bodyCode = 'LOU';
		userData.bodyNameAscii = 'Louvain-la-neuve';
		userData.uid = userData.name + '.' + userData.sn;
		//userData.userId = userData.name + '.' + userData.sn;
		//my static id
		// var uid = req.user.uid;
		
		var options = {
		  'path': '/users/'+uid+'/memberships/create',
		  'headers': {
			'x-access-token': token.APItoken
		  }
		};
		...
		restClient.post(options, userData, function(resterr, restreq, restres, restobj) {
		  ...
		  var message = { 'outcome': 'success', 'message': 'Member Register'}
	...
	function changeMemberType(req,res,next)
	{
	  /*
	   * code of modify membership
	   */
	  var uid = req.body.name+'.'+req.body.sn;
	  var data = {};
	  data.memberType = "Member";
	  var options = {
	  'path': '/users/'+uid+'/memberships/LOU/modify',
	  'headers': {
			'x-access-token': token.APItoken
		  }
	   };
	  //use API
	  restClient.post(options, data, function(resterr, restreq, restres, restobj) {
			console.log('%d -> %j', restres.statusCode, restres.headers);
			console.log('%j', restobj);  
			return next();
	  });		
...
// My code
	...
	router.get('/members/add',isLoggedIn, renderNewMember);
	router.post('/newMember',isLoggedIn, addNewMember, changeMemberType,
			function(req,res){
			  res.redirect('/members/LOU');
			});
